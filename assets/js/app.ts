...
class App {
    ...
    add(text:string) {
        ...
        var todo = new Todo(this.todoID,text,TodoStatus.TODO);
        this.todoID += 1;
        this.entries.push(todo);
        this.save();
        App.log('added %s (%d)', text, todo.ID);
    ...
    buildHTMLTodo(todo:Todo):HTMLElement {
        ...
        text.addEventListener('dblclick',(e:MouseEvent)=>{
            e.preventDefault();
            App.log('editing todo with ID %d',todo.ID);
            this.edit(todo.ID,li);
        },false);

        confirmButton.addEventListener('click',(e:MouseEvent)=>{
            e.preventDefault();
            App.log('confirming todo with ID %d',todo.ID);
        ...
        removeButton.addEventListener('click',(e:MouseEvent)=>{
            e.preventDefault();
            App.log('removing todo with ID %d',todo.ID);
